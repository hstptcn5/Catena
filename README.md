...
[![CI](https://github.com/hstptcn5/Catena/actions/workflows/ci.yml/badge.svg)](https://github.com/hstptcn5/Catena/actions/workflows/ci.yml)

**Turn any SQLite file into a realtime HTTP and WebSocket API.**

Catena sits in front of a normal SQLite file and gives it a network API: run parameterized SQL over HTTP, receive realtime notifications for writes made through Catena over WebSocket, inspect it from an embedded admin UI, and export or back it up when needed.
...
- `GET /ws` for table-change notifications generated by writes made through Catena.
- API key authentication.
- Optional trusted named commands with closed input schemas, per-command
  permissions, inbound idempotency, atomic receipts, and Anetac outbox inserts.
- Optional `--disable-raw-sql` integrated mode.
- Read-only mode.
...
## Catena Node integration

Catena remains independently useful as SQLite-over-HTTP. When
`--commands-file` and `--command-auth-file` are supplied, it additionally
exposes:

| Endpoint | Method | Purpose |
| --- | --- | --- |
| `/v1/commands/{name}` | `POST` | Execute one authenticated, idempotent trusted command |
| `/v1/receipts/{command_id}` | `GET` | Read local commit and outbound delivery state |

The business statements, result snapshot, command receipt, Anetac outbox rows,
and command-event links commit in one SQLite transaction. A retry with the same
`Idempotency-Key` and normalized input returns the stored command ID and result;
different input returns `409`.

This mode requires an Anetac schema with optional command/correlation/causation
columns. It does not make standalone Catena require Anetac. See
[Named Commands and Receipts](docs/NAMED_COMMANDS.md).
